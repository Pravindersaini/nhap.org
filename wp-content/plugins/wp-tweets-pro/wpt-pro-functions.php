...
<div class="wrap" id="wp-to-twitter" >
						...
						if ( $hook == 'wpt_schedule_tweet_action' ) {
							...
							if ( ( isset( $_GET['wpt'] ) && $_GET['wpt'] == 'clear' ) && ( isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce']) ) ) {
								...
								echo "<div id='message' class='updated'><p>".sprintf(__('Tweet for %1$s has been deleted.','wp-tweets-pro'),date( $date_format, ($timestamp+$offset) ))."</p></div>";
										...
										isset($_GET['ts']) && $_GET['ts'] == $timestamp &&
										isset($_GET['auth']) && $_GET['auth'] == $auth &&
										isset($_GET['rt']) && $_GET['rt'] == $rt &&
										isset($_GET['rp']) && $_GET['rp'] == $post_ID &&
										isset($_GET['hash']) && $_GET['hash'] == md5($sentence) ) {
								wp_unschedule_event( $timestamp, 'wpt_schedule_tweet_action', array( 'id'=>$auth,'sentence'=>$sentence,'rt'=>$rt,'post_id'=>$post_ID ) );
	...
	<div class="postbox">
		...
		<?php $nonce = wp_nonce_field('wp-to-twitter-nonce', '_wpnonce', true, false).wp_referer_field(false);  echo "<div>$nonce</div>"; ?>	
			...
			<?php $last = wp_get_recent_posts( array( 'numberposts'=>1, 'post_type'=>'post' ) ); $last_id = $last['0']['ID']; ?>
			<p>
...
function wpt_get_past_tweets() {
	...
	$offset = ( isset($_GET['paged']) && $_GET['paged'] != 1 ) ? 50*$_GET['paged'] : 0;
	$posts = new WP_Query( array( 'posts_per_page'=>50, 'offset'=>$offset, 'post_type'=>$post_type, 'meta_key'=>'_jd_wp_twitter', 'meta_query'=>array( array( 'key'=>'_jd_wp_twitter', 'compare'=>'EXISTS' ) ) ) );
	$output = '';
...
function wpt_get_failed_tweets() {
	...
	$post_type = ( isset($_GET['ptype']) )?$_GET['ptype']:'post';
	$offset = ( isset($_GET['paged']) && $_GET['paged'] != 1 ) ? 50*$_GET['paged'] : 0;
	$posts = new WP_Query( array( 'posts_per_page'=>50, 'offset'=>$offset, 'post_type'=>$post_type, 'meta_key'=>'_wpt_failed', 'meta_query'=>array( array( 'key'=>'_wpt_failed', 'compare'=>'EXISTS' ) ) ) );
	$output = '';
	while( $posts->have_posts() ) {
		$posts->the_post();
		$post = get_post( get_the_ID() );
	...
	<div class="inside">
		<?php
			if ( isset( $_GET['ptype'] ) ) { 
				...
				<p>
					<a href="<?php echo $url; ?>"><?php _e('Clear deleted Tweets for this post type','wp-tweets-pro'); ?></a>
				</p>
			<?php } ?>	
	<p><?php _e('This is a list of tweets WP Tweets PRO failed to send to Twitter with the reported reason for the failure. Data gathered starting with WP to Twitter version 2.4.7. Each page shows the failed tweets in that set of 50 posts.','wp-tweets-pro'); ?>
	<?php echo $types; ?>
	<?php
...
function wpt_insert_post_info( $values, $post_ID ) {
	if ( is_array( $values ) ) {
		...
		$image = ( isset( $_POST['_wpt_image'] ) ) ? 1 : false;
...
function wpt_check_license( $key=false ) {
	...
	if ( !$key ) {
		...
		define('WPT_PRO_PLUGIN_LICENSE_URL', "http://www.joedolson.com/articles/wp-content/plugins/files/license.php" );
...
function wpt_pro_menu() {
  ...
  get_currentuserinfo();
  $allowed_group = apply_filters( 'wpt_main_settings', 'update_core', $user_ID );
...
add_filter( 'wpt_save_user','wpt_update_user_templates',10,2 );
function wpt_update_user_templates( $edit_id, $post ) {
	if ( function_exists( 'wpt_pro_exists' ) ) {
...
function wpt_twitter_user_fields( $edit_id ) {
	...
	return sprintf( "
		...
		<tr><th scope='row'><label for='wpt_edit'>".__('Tweet Template for edited posts:','wp-tweets-pro').'</label></th><td><input type="text" class="regular-text" name="wpt_templates[edit]" id="wpt_edit" value="%2$s" /></td></tr>', $new, $edit );
}
...
function wpt_schedule_values( $post_id, $display='normal' ) {
	...
	if ( $display == 'hidden' ) {
	?>
	<input type='hidden' name='_wpt_cotweet' value='<?php if ( $cotweet != false ) { echo ( $cotweet == 1 )?'on':''; } else { echo ( get_option('wpt_cotweet') == 1 )?'on':''; } ?>' />
...
<p>
<?php $checked = ( $cotweet == false )?jd_checkCheckbox( 'wpt_cotweet' ):' checked="checked"'; ?>
...
<?php if ( get_option( 'wpt_media' ) == 1 ) { ?>
<p>
<input type='checkbox' name='_wpt_image' id='wpt_image' value='1' /> <label for='wpt_image'><?php _e('Do not upload an image.','wp-tweets-pro'); ?></label>
</p>
<?php } ?>
<p>
<label for="wrr"><?php _e("Re-post how many times:", 'wp-tweets-pro'); ?></label> 
	<select name="_wpt_retweet_repeat" id="wrr">
		<option value="0"<?php echo wtt_option_selected( $repeat,'0','option' ); ?>>0 </option>	
		<option value="1"<?php echo wtt_option_selected( $repeat,'1','option' ); ?>>1 </option>
		<option value="2"<?php echo wtt_option_selected( $repeat,'2','option' ); ?>>2 </option>
		<option value="3"<?php echo wtt_option_selected( $repeat,'3','option' ); ?>>3 </option>
	</select>
</p>
<?php
	}
...
if ( get_option( 'wpt_license_valid' ) == 'true' ) {
	...
	$message = sprintf(__("You must <a href='%s'>enter your WP Tweets Pro license key</a> to use your PRO features with WP to Twitter. .", 'wp-tweets-pro'), admin_url('admin.php?page=wp-tweets-pro'));
...
function wpt_authorized_users( $selected=array() ) {
	...
	$select = "<p class='wpt_auth_users'><label for='wpt_authorized_users'>Tweet to:</label><br /><select multiple='multiple' name='_wpt_authorized_users[]' id='wpt_authorized_users'>";
	if ( !empty( $users ) ) {
		foreach ( $users as $user ) {
			...
			if ( $id == $current || in_array( $id, $selected ) ) { $active = " selected='selected'"; }
...
function wpt_check_for_upgrades() {
    ...
    $wpt_plugin_remote_path = "http://www.joedolson.com/articles/wp-content/plugins/files/updates-v2.php?key=$hash";
